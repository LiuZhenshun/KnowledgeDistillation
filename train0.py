...
def main_worker(gpu, ngpus_per_node, argss):
    ...
    if args.arch == 'kd':
        from model.kd_model import kd_model
        model = kd_model(classes=args.classes, criterion=criterion)
        modules_ori = [model.layer0]
        modules_new = [model.ppm, model.conv2d, model.cls]
...
def train(train_loader, model, optimizer, epoch):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        output, main_loss, aux_loss, aux_loss2 = model(x=input, y=target)
        if not args.multiprocessing_distributed:
            main_loss, aux_loss = torch.mean(main_loss), torch.mean(aux_loss)
        loss = main_loss + args.aux_weight * aux_loss + args.aux_weight * aux_loss2
